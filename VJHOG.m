%% PART 1 :Capture Photo for user and store it in database
...
%creating video player object
videoPlayer = vision.VideoPlayer('Position', [100 100 [frameSize(2), frameSize(1)]+30]);



%% PART 2: Detecting face and Tracking
runLoop = true;
numPts = 0;
frameCount = 0;
...
%% PART 3 : Save image Captured in Database
...
%%each time capture face label and save it in the corresponding folder, do 
%from part1 to capture new image)

        imwrite(getimage,'H:\Ramithaa VJHOG2\VJHOG\Database\Ramithaa\.jpg'); 


%% Part 4: Train Image captured in the databse folder
%%Load Image Information from ATT Face Database Directory
faceDatabase = imageSet('H:\Ramithaa VJHOG2\VJHOG\Database\','recursive');
...
%% Part 5 : Predict Person in the Database folder
...
videoPlayer = vision.VideoPlayer('Position', [100 100 [frameSize(2), frameSize(1)]+30]);
%%%Run Video for Predicition

runLoop = true;
numPts = 0;
frameCount = 0;
%pic = webcam('Integrated picera');
while runLoop && frameCount < 300
    ...
    if numPts < 10
        ...
        if numPts >= 10
            ...
            maxpro = max(abs(PostProbs(1)),abs(PostProbs(1)));
